...
var utils = {
	...
	applyTextShadows (style, shadows) {
		const map = shadows.map((shadow) => `${shadow.x} ${shadow.y} ${shadow.blur} ${getColorString(shadow.color)}`);
		style.textShadow = map.toString();
	},

	applyBoxShadows (style, shadows) {
		const map = shadows.map((shadow) => `${shadow.type === 'inset' ? 'inset ' : ''}${shadow.x} ${shadow.y} ${shadow.blur} ${shadow.spread} ${getColorString(shadow.color)}`);
	...
	resolveReportReason(reportArray){
		const reportCounts = countBy(reportArray, function (item) {
			if(!item.reason || !/广告|侵权|欺诈|色情|侮辱诋毁/.test(item.reason)){
				item.reason = '其他';
			}
			return item.reason;
		});
		return [
			{label: '广告', count: reportCounts['广告'] || 0},
			{label: '侵权', count: reportCounts['侵权'] || 0},
			{label: '欺诈', count: reportCounts['欺诈'] || 0},
			{label: '色情', count: reportCounts['色情'] || 0},
			{label: '侮辱诋毁', count: reportCounts['侮辱诋毁'] || 0},
			{label: '其他', count: reportCounts['其他'] || 0}
		]
	}
};
