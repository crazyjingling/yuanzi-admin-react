...
export default {
	...
	async resolve (root, params, options) {
		...
		if(params.data.account.platform === '微博'){
			params.data.account.platform = 'weibo'
		}if(params.data.account.platform === '后台'){
			params.data.account.platform = 'local'
		}if(params.data.account.platform === 'QQ'){
			params.data.account.platform = 'qq'
		}if(params.data.account.platform === '手机'){
			params.data.account.platform = 'mobile'
		}if(params.data.account.platform === '微信'){
			params.data.account.platform = 'weixin'
		}
		...
		if(params.data.isDel === '封号'){
			params.data.isDel = true
		}else {
			params.data.isDel = false
		}
		...
		await ArticleModel.update({owner: params.data._id}, {'$set': { 'isDel': true}},false,true).exec();
		await WorkModel.update({owner: params.data._id}, {'$set': { 'isDel': true}},false,true).exec();
		await CommentModel.update({commentUser: params.data._id}, {'$set': { 'isPassed': false}},false,true).exec();
