...
const activityType = new GraphQLObjectType({
	...
	fields: {
		...
		pictureContent: {type: new GraphQLObjectType({
			name: 'pictureContentActivityCover',
			fields: {
				_id: {
					type: GraphQLString,
					resolve: (media, params, options) => media._id || ''
			...
			async resolve (schemaEntry, params, options) {
				const media = await MediaModel.findOne({ossUrl: schemaEntry.pictureContent}).select({_id: 1, ossUrl: 1}).exec();
				return {_id: media ? media._id : '', ossUrl: schemaEntry.pictureContent};
			}},
