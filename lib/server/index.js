...
app.use('/changelog', (req, res, next) => {
	console.log("1")
	var fs = require('fs');
	marked.setOptions({
	...
	fs.readFile('./CHANGELOG.md',function (error, fileData) {
		var fileData = fileData.toString('utf8', 0, fileData.length);
		res.status(200).send(marked(fileData));
	});
