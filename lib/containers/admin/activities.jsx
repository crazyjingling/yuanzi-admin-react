...
@connect(
	(state) => ({
		...
		count: state.activities.data.count
	}),
	(dispatch) => bindActionCreators(activitiesActions, dispatch)
)
@queryProps({
	page: 1,
	limit: 20,
...
export default class ActivitiesContainer extends Component {
	...
	onAddNew(newActivity) {
		this.props
			.addActivity({activity: Activities.fragments.activities}, newActivity)
			.then(() => {
